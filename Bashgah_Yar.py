...
class ROOT(QMainWindow):
    ...
    def userNameChek(self, username):
        self.con = sqlite3.connect(MAIN_DB_FILE)
    ...
    def about_me_small(self):
        QMessageBox.information(QMessageBox(),"در باره برنامه","برنامه باشگاه یار ورژن 4.4.2  \nساخته وتوسعه یافته توسط artinZZ")
...
class INFOBUP(QDialog):
    ...
    def aadAccept(self):
        ...
        if file_path:
            engineA = create_engine(f"sqlite:///{MAIN_DB_FILE}")
    ...
    def mmdAccept(self):
        ...
        if file_path:
            engineM = create_engine(f"sqlite:///{MAIN_DB_FILE}")
