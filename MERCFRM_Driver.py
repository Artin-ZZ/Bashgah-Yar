    ...
    def __init__(self, *args, **kwargs):
        ...
        if os.path.exists(MAIN_DB_FILE):
            try:
                ...
                print(f"Error hiding {MAIN_DB_FILE}: {e}")
        else:
    ...
    def searchReqs(self):
        ...
        if not fname:
            ...
            try:
                ...
                result = self.c.execute("SELECT fullname, date, req_item, price, regerfnm, saleno, req_stat FROM salesinfo WHERE fullname = ?", (fname,))
...
class DeleteDialog(QDialog):
    def __init__(self, *args, **kwargs):
        ...
        self.deleteinput.setPlaceholderText("شماره درخواست را وارد کنید")
    ...
    def deleteUser(self):
        ...
        if not delrol:
            ...
            try:
                self.conn = sqlite3.connect(MAIN_DB_FILE)
                self.c = self.conn.cursor()
                self.c.execute("DELETE FROM salesinfo WHERE saleno = ?", (delrol,))
                self.conn.commit()
                self.c.close()
                self.conn.close()
